...
class TestUpdateProfile:
    def test_update_basic_info(self):
        ...
        profile = update_profile("user1", update)
    ...
    def test_update_preserves_existing(self):
        ...
        update_profile("user1", update1)
        update2 = OnboardingUpdate(weight_kg=80.0)
        profile = update_profile("user1", update2)
...
class TestAnalyzeBodyPhoto:
    def test_analyze_returns_success(self):
        result = analyze_body_photo("user1", b"fake_image_data")
...
class TestCompleteOnboarding:
    def test_complete_existing_profile(self):
        update = OnboardingUpdate(height_cm=170.0)
        update_profile("user1", update)
        result = complete_onboarding("user1")
